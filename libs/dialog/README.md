...
dialog 提供了一个增强版用来支持复杂的 iframe 套嵌的页面，可以在顶层页面创建一个可供 iframe 访问的对话框创建方法，例如：
...
打开配置文件： src/dialog-config.js，其中``cssUir``字段是 css 文件的路径，``innerHTML``字段则是 dialog 的模板。修改这两个字段即可很方便的设计属于自己的皮肤。

一套皮肤可以添加不同的``className``实现多种状态，可参考 [skin](#option-skin) 选项。

###	[源码构建](id:other-grunt)

使用 [GruntJS](http://gruntjs.cn) 在 dialog 源码根目录执行即可。


###	[支持](id:other-support)

dialog 是基于 LGPL 协议免费开源的程序。
