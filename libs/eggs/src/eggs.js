define(function (require, exports, module) {
	...
	eggs.fn.getTpl = function (id) {
		return [
				'<div class="golden_egg" id="'+id+'">',
				'<span class="act_golden_close"></span>',
				'<ul class="golden_ul">',
				'<p class="hammer pic pic_5" id="hammer">锤子</p>',
				'<p class="resultTip" id="resultTip"><b id="result"></b></p>',
				'<li class="golden_1"><span class="pic pic_1"></span></li>',
				'<li class="golden_2 center"><span class="pic pic_1"></span></li>',
				'<li class="golden_3"><span class="pic pic_1"></span></li>',
				'</ul>',
				'</div>'
		].join('');
	};
	...
	eggs.fn.script = function (options) {
		...
		function eggClick(obj) {
			var _this = obj;
			$.post(options.url+"bs/prize/check/hitegg", {id:options.orderId},function (res) {
				...
				$(".hammer").show().css({"top": _this.position().top - 85, "left": _this.position().left + 185});
				$(".hammer").animate({
						...
						_this.find('span.pic').attr("class", "pic " + cls); //蛋碎效果
						...
						$('.resultTip').css({display: 'block', top: '100px', left: _this.position().left + 45, opacity: 0}).animate({top: '50px', opacity: 1}, 300, function () {
							if (res.state == 0) {
								...
								_fn.tips.no(options);
								//$("#result").html(res.prize || "很遗憾,您没能中奖!");
							}
						});
	...
	eggs.fn.tips.getTpl = function(type,options){
		switch (type){
			case 'ok':
				return [
					'<div id="ks-content-ks-component" class="ks-overlay-content">',
					'	<div class="wrapper">',
					'	<div class="layer-ok layer-ie-ok">',
						'<i class="layer-money" target="_blank">'+options.money+'</i>',
						'<i class="layer-close"></i>',
					'		<a href="'+options.url+'act/'+options.orderId+'/award.html" class="layer-link" target="_blank">填写领奖信息</a>',
					'	</div>',
					'</div>',
					'</div>'
				].join('');
			...
			case 'no':
				return [
					'<div id="ks-content-ks-component" class="ks-overlay-content">',
					'	<div class="wrapper">',
					'	<div class="layer-no layer-ie-no">',
						'		<i class="layer-close"></i>',
					'		<a href="javascript:void(0);" class="layer-link">下次再来</a>',
					'	</div>',
					'</div>',
					'</div>'
				].join('');
